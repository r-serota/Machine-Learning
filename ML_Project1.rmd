...
dataSVM <- dataSVM[-sample(nrow(dataSVM),1),] # I excluded one observation in order to have the same size of training and test set. Otherwise, there was a problem to calculate the confusion matrix. TODO: excluding one observation randomly

# dividing our dataset into training and test sets (50%/50%)
train = sample(nrow(dataSVM), 0.5*nrow(dataSVM))
...
tune.out = tune(svm, factor(studentPerformance) ~ ., data = dataSVM[train, ], kernel = "radial",
...
table(true = dataSVM[-train, ]$studentPerformance,  pred = predict(svmfit.opt, newx = dataSVM[-train, ]))

SVMErrorRate <- (1 - mean(predict(svmfit.opt, newx = dataSVM[-train, ]) == dataSVM[-train, ]$studentPerformance)) # error rate

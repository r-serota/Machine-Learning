...
>>>>>>> some summary + changes in logit and svm
```
...
Let's now compare averaged accuracy (measured by AUC) for default Random Forest and Bagging method. Both errors were estimated on the test data.
```{r }
auc_value20 = 0.0
repeat_each = 20
...
perf = performance(predob, "tpr", "fpr")
plot(perf, main = "Boosting with B = 760", colorize = TRUE) 
...
For the first task, predicting if student will pass the exam, five methods were used: Logistic Regression, KNN, Random Forest, Linear Description Analysis and Support Vector Machines. Every method has been tested on validation test, which was randomly selected in proportion of 20/80(TODO - make sure it's the truth). Also, for method that support variaple importance, this analysis was conducted. Results are presented in a table below.

Algorithm | AUC | Error rate
------------- | ------------- | ------------- 
Random forest |0.8011636 | 
SVM | - | 0.2692308 | 
Logistic regression | 0.7886 | TODO 
LDA | TODO | TODO 
KNN | TODO | TODO 

Also, random forest resulted in a variable importance summary with respect to predicting if student will pass the exam. Most 5 important variables were: number of absences, mothers education, student health (measured by 1-5 scale), reason to choose particular school, weekly study time.

<!-- 
  I didn't conclude boosting results here, cause it's a part of random forest 
  Boosting AUC = 0.7820606
  Boosting importance variables: failures, Medu,, absences, reason
-->

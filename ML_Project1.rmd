...
We use logistic regression and calculate its error rate on a train data.
```{r }
...
```
On the training data, the Logistic regression model achieved the success rate equal to 81.92%. However, because of frequent occurance of overfitting, it is better to compare algorithms' scores on the test set.
...
In order to determine if the obtained statistics is good or not, we compare the obtained result with the base model which is a reflection of a random guessing.
```{r }
...
```
The random guessing models success rate is equal to 69,77%. It follows that the logistic regression model gives us an improvement of the success rate at the level of 5.4 percentage points.

Next, we visualize predictors accuracy relative to the threshold, using ROC curve.
...
Let us now compare averaged accuracy (measured by AUC) for default Random Forest and Bagging method. Both errors were estimated on the test data.
```{r }
auc_value20 = 0.0
repeat_each = 20
...
At the beginning, we used Cross-Validation which enabled us to pick a proper number of trees equal to 760. According to this value, we built a Boosting model, plotted the ROC curve, calucalted the AUC and presented which variables were considered as the most important.
